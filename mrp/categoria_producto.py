...
def index():
    ...
    if busqueda == '':
        datos = db.paginate(db.select(CategoriaProducto).order_by(CategoriaProducto.nombre.desc()), per_page=5, page=pagina)
    else:
        datos = db.paginate(db.select(CategoriaProducto).filter(CategoriaProducto.nombre.ilike(f'%{busqueda}%')), per_page=5, page=pagina)
    return render_template('categoria_producto/index.html', datos=datos, pagina=pagina, busqueda=busqueda)
...
def csv_import_all():
    if request.method == 'POST':
        ...
        next(reader)
        # Cargo todos los categorías existentes, por si existe un código previamente ignoralo
        lista_categorias = CategoriaProducto.query.all()
        # convierto la lista en un diccionario (el código debe ser único)
        dict_categorias = {categoria.codigo: categoria for categoria in lista_categorias}

        nuevos = 0
        existentes = 0

        for row in reader:
            if row['codigo'] in dict_categorias:
                existentes = existentes + 1
            else:
                categoria = CategoriaProducto(codigo=row['codigo'], nombre=row['nombre'],
...
def csv_export_all():
    ...
    datos = CategoriaProducto.query.order_by(CategoriaProducto.nombre.desc()).all()
    ...
    response.headers['Content-Disposition'] = 'attachment; filename="categoria_producto_export_all.csv"'
    return response
...
def csv_template():
    ...
    response = Response(file_output)
    response.headers['Content-Type'] = 'text/csv; charset=utf-8'
    response.headers['Content-Disposition'] = 'attachment; filename="categoria_producto_template.csv"'
    return response
